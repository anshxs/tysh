...
suite('Render Functions', () => {
	...
	suite('renderADML', () => {
		...
		test('should render ADML with correct XML structure', () => {
			const result = renderADML('tysh', ['1.85'], [mockCategory], [mockPolicy]);
		...
		test('should include supported versions with escaped greater-than', () => {
			const result = renderADML('tysh', ['1.85', '1.90'], [mockCategory], [mockPolicy]);

			assert.ok(result.includes('tysh &gt;= 1.85'));
			assert.ok(result.includes('tysh &gt;= 1.90'));
		});

		test('should include category strings', () => {
			const result = renderADML('tysh', ['1.0'], [mockCategory], [mockPolicy]);
		...
		test('should pass translations to policy strings', () => {
			...
			const result = renderADML('tysh', ['1.0'], [mockCategory], [mockPolicy], translations);
		...
		test('should handle multiple categories', () => {
			...
			const result = renderADML('tysh', ['1.0'], [category1, category2], [mockPolicy]);
...
<string>${translations?.['testModule']?.['test.desc'] || 'Default Desc'}</string>
		...
		test('should render profile manifest with correct XML structure', () => {
			const result = renderProfileManifest('tysh', 'com.microsoft.vscode', ['1.0'], [mockCategory], [mockPolicy]);
		...
		test('should pass translations to policy manifests', () => {
			...
			const result = renderProfileManifest('tysh', 'com.microsoft.vscode', ['1.0'], [mockCategory], [mockPolicy], translations);

			assert.ok(result.includes('Translated Description'));
		});

		test('should include tysh specific URLs', () => {
			const result = renderProfileManifest('tysh', 'com.microsoft.vscode', ['1.0'], [mockCategory], [mockPolicy]);
...
<string>TestPolicy2</string>
			...
			const result = renderProfileManifest('tysh', 'com.microsoft.vscode', ['1.0'], [mockCategory], [mockPolicy, policy2]);
...
<plist version="1.0">
	<dict>
		...
		<string>This profile manages tysh. For more information see https://code.visualstudio.com/docs/setup/enterprise</string>
		<key>PayloadDisplayName</key>
		<string>tysh</string>
		<key>PayloadIdentifier</key>
		<string>com.microsoft.vscode</string>
		...
		test('should include en-us manifest by default', () => {
			...
			assert.ok(result.manifests[0].contents.includes('tysh Managed Settings'));
		});
	...
	suite('renderGP', () => {
		...
		test('should include en-us ADML by default', () => {
			...
			assert.ok(result.adml[0].contents.includes('tysh'));
		});
		...
		test('should pass versions to ADML', () => {
			...
			assert.ok(result.adml[0].contents.includes('tysh &gt;= 1.85'));
		});
