...
/// Number of seconds where, if there's no connections to a tysh server,
...
/// served the latest version of the tysh server whenever they load the
/// page. The tysh server prefixes all assets and connections it loads with
/// its version string, so existing clients can continue to get served even
/// while new clients get new tysh Server versions.
...
/// Gets the release info from the tysh path prefix, which is in the
...
impl ConnectionManager {
	...
	/// Gets the StartData for the a version of the tysh server, triggering
