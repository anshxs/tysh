...
export function getAccessibilityHelpText(type: 'panelChat' | 'inlineChat' | 'quickChat' | 'editsView' | 'agentView', keybindingService: IKeybindingService, supportsFileReferences: boolean, isSessionsWindow: boolean = false, stickyPromptHeaderShown: boolean = false, sessionStatusPillsSupported: boolean = type === 'panelChat' || type === 'agentView', sessionArchiveNudgeShown: boolean = false, sessionArchiveActionWording = ChatSessionArchiveActionWording.Archive): string {
	...
	if (isSessionsWindow) {
		...
		content.push(localize('chat.testApp.help', "After app UI changes, Test App appears to the right of the status pills above the chat input. After a testing request starts in this chat, the button is named Retest App whenever it reappears, including after restarting tysh in the same profile. This remembers that testing was requested, not that tests passed. Press Shift+Tab from the input to focus it, then Enter or Space to ask the agent to review the diffs, test the app UI, fix issues found, and retest. Existing tool permissions still apply."));
	}
	...
	if (type === 'panelChat' || type === 'quickChat' || type === 'agentView') {
		...
		content.push(localize('chat.completedResponseDisclosure', 'When completed response collapsing is enabled, the final response remains visible while earlier work is collapsed. Use Tab to focus the work disclosure and press Enter or Space to show or hide that work. With persistent progress enabled, a separate file changes button shows lines added and deleted when the collapsed work includes edits. Activate it to open those file changes without expanding the work.'));
		if (type === 'agentView') {
		...
		content.push(localize('chat.persistentProgress', "The chat.experimental.persistentProgress setting defaults to Off, preserving the original thinking and progress UI. Choosing Weave, Draw, Orbit and Lock, Accordion, or Dial Rotation keeps a working progress indicator at the bottom until the response finishes. Tool chains stay expanded without a group header or an internal scrolling limit. Completed responses still follow chat.agent.collapseCompletedResponses; expanding the completed-steps disclosure reveals those same tool chains. Reasoning appears separately as collapsible previews; focus a reasoning header and press Enter or Space to expand or collapse it. Standalone tools keep their decorative icons. Terminal activity animations and subagent pills are unchanged. While the response waits on subagents or background agents, the indicator reports how many subagents it is waiting for; while it waits on a background terminal, it reports that it is waiting for terminal output. Reduced motion keeps the indicator visible without animation. Spoken progress updates are controlled by accessibility.verboseChatProgressUpdates."));
	...
	if (type === 'editsView' || type === 'agentView') {
		...
		if (type === 'agentView') {
			...
			content.push(localize('chatAgent.acceptTool', "To accept a tool action, use the Accept Tool Confirmation command{0}. When the confirmation carousel is shown, this activates the primary button of the selected confirmation. Disabled actions cannot be accepted.", '<keybinding:workbench.action.chat.acceptTool>'));
			content.push(localize('chatAgent.sectionProgress', "Finished thinking and tool-call sections stop showing activity. The Working indicator is omitted when subagent pills are the last visible content; it appears after other content while the response remains in progress."));
