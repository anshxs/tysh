...
export const chatProgressConfigurationProperties = {
	[ChatConfiguration.PersistentProgress]: {
		...
		enumDescriptions: [
			localize('chat.progressAnimation.off', "Keep the original thinking, tool, and working progress rendering without a persistent indicator or tysh logo."),
		...
		markdownDescription: localize('chat.experimental.persistentProgress', "Keep a working progress indicator with an animated tysh logo at the bottom until the response finishes. Selecting an animation shows tool calls in expanded, headerless chains without an internal scrolling limit. Reasoning is separated into collapsible previews that break the tool chain, and standalone tools retain their icons. Completed responses still follow the {0} setting. This replaces inner working progress; terminal activity animations and rich subagent pills are unchanged. Off preserves the original thinking, tool, and progress rendering. Changes apply immediately; reduced motion keeps the indicator visible without animation.", `\`#${ChatConfiguration.CollapseCompletedResponses}#\``),
		tags: ['experimental'],
