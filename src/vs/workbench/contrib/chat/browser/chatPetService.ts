...
export const ChatPetContextKeys = {
	enabled: new RawContextKey<boolean>('chatPetEnabled', false, localize('chatPet.context.enabled', "Whether the tysh pet is enabled")),
};
...
type ChatPetEnablementClassification = {
	owner: 'justschen';
	comment: 'Tracks tysh pet enablement so adoption can be measured.';
	enabled: { classification: 'SystemMetaData'; purpose: 'FeatureInsight'; comment: 'Whether the tysh pet is enabled.' };
	source: { classification: 'SystemMetaData'; purpose: 'FeatureInsight'; comment: 'Whether the state was observed at startup or changed while tysh was running.' };
};
...
export class ChatPetService extends Disposable implements IChatPetService {
	...
	toggle(): boolean {
		...
		status(enabled
			? localize('chatPet.enabled', "tysh pet enabled. Click the pet to interact with it, or use the Left and Right Arrow keys to move it.")
			: localize('chatPet.disabled', "tysh pet disabled"));
		return enabled;
	}
	...
	setVariant(variant: ChatPetVariant): void {
		...
		status(variant === 'stable'
			? localize('chatPet.variant.stable', "tysh pet changed to the Stable colors")
			: localize('chatPet.variant.insiders', "tysh pet changed to the Insiders colors"));
	}
	...
	setOnTheRun(onTheRun: boolean): void {
		...
		status(onTheRun
			? localize('chatPet.onTheRun', "The tysh pet is on the run. Click the pet to bring it back.")
			: localize('chatPet.restored', "The tysh pet is back"));
	}
