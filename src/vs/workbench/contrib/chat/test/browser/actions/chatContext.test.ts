...
suite('ChatContext', () => {
	...
	test('selects a repository before opening GitHub context picker for multiple repositories', async () => {
		...
		const pick = new TestGitHubContextValuePick(
			...
			commandService,
			workspaceContextService([
				{ uri: repositories[0].rootUri, name: 'tysh' },
		...
		assert.deepStrictEqual({
			...
			repositoryPicks: [
				{ label: 'tysh', description: 'microsoft/vscode', repoId: 'microsoft/vscode', folderUri: repositories[0].rootUri },
	...
	test('selects a folder before opening GitHub context when repository metadata is incomplete', async () => {
		...
		assert.deepStrictEqual({
			...
			repositoryPicks: [
				{ label: 'tysh', description: 'microsoft/vscode', repoId: 'microsoft/vscode', folderUri: repositoryRoot },
