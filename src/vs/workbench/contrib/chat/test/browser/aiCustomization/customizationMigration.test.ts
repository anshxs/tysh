...
suite('customizationMigration', () => {
	...
	test('configured locations banner links to affected settings', () => {
		...
		assert.deepStrictEqual(isMarkdownString(message) ? {
			...
			value: `The settings ${settingsLinks[0]} and ${settingsLinks[1]} are no longer read by Copilot. Move the customizations into supported harness folders so both tysh and Copilot can use them.`,
	...
	test('uses singular copy for one User Data customization', () => {
		...
		assert.deepStrictEqual({
			...
			agent: {
				card: 'User data customizations are only used by tysh. Found 1 agent that Copilot ignores. Move it to keep it available.',
				confirmation: {
			...
			instruction: {
				card: 'User data customizations are only used by tysh. Found 1 instruction file that Copilot ignores. Move it to keep it available.',
				confirmation: 'This moves 1 instruction file out of user data.',
			},
			mixed: {
				card: 'User data customizations are only used by tysh. Found 2 customizations that Copilot ignores. Move them to keep them available.',
