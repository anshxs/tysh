...
/**
 * Auto-trusts every isolated git worktree in `workspace` that tysh created off
 ...
 *   not prove tysh created the worktree (its contract permits an arbitrary
...
export async function ensureSessionWorktreesTrusted(
	...
	await Promise.all(workspace.folders.map(async folder => {
		...
		// root), but on its own it does not prove tysh created that worktree.
