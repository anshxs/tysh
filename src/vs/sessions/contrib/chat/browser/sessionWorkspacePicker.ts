...
export class WorkspacePicker extends Disposable {
	...
	constructor(
		...
		// tysh's recent-workspace history is loaded asynchronously.
	 ...
	 * without source grouping. Own recents come first, followed by tysh
	 * recent folders.
	 */
	protected _buildItems(): IActionListItem<IWorkspacePickerItem>[] {
		...
		// Own recents first, then tysh recents (merged and deduplicated by the service)
	...
	private _restoreSelectedWorkspace(): IRestoredWorkspaceSelection | undefined {
		...
		// Agents-owned recents are ordered before tysh's general recents.
		try {
