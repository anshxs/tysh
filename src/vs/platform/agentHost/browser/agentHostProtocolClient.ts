...
// AHP 0.9 changed the automation catalog wire shape, so tysh cannot safely negotiate 0.8.
...
export class AgentHostProtocolClient extends Disposable implements IAgentConnection, IRemoteAgentHostProtocolClient {
	...
	private _handleMessage(msg: ProtocolMessage): void {
		...
		if (isJsonRpcRequest(msg)) {
			...
			switch (msg.method) {
				case 'action': {
					// Protocol envelope → tysh envelope (superset of action types)
	...
	/** Send a JSON-RPC request for a tysh extension method (not in the protocol spec). */
