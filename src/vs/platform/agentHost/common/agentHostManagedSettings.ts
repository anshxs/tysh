...
/**
 * Compatibility bridge from legacy tysh settings to Copilot SDK managed
 * settings.
 *
 ...
 *   source supplies one, so a lone list from tysh would grant blanket
 ...
 * mappings whose tysh behavior already honors user and application values,
...
/**
 * Translates tysh's agent network filter into managed domain rules.
 *
 * Only the blocking half is expressible. tysh denies any domain outside a
...
function contributeNetworkDomainRules(configurationService: IConfigurationService): IAgentHostManagedSettingsPermissions | undefined {
	...
	// tysh's restrictive default: with the filter on and neither list
	...
	for (const pattern of denied) {
		...
		// rule from it. tysh matches on the hostname alone, so a denial written
		...
		// tysh accepts a bare `*` as "every domain"; the SDK expresses that as
...
/**
 * Translates tysh's explicit terminal auto-approve denials into managed
 * shell rules.
 *
 ...
 * tysh escapes `*` as a literal character while the SDK reads it as a
...
const managedPermissionsSettings: readonly IManagedPermissionsSettingMapping[] = [
	...
	// Matches tysh, where a user or application value already suppresses
	...
	// The filter and its lists are evaluated together, and tysh honors a user
