...
export class NetworkDiagnosticsService implements INetworkDiagnosticsService {
	 ...
	 * traverses the same proxy / TLS / certificate stack the rest of tysh
	...
	async fetch(url: string): Promise<IAgentHostNetworkFetchResult> {
		...
		// Reachability: a GET through IRequestService, which applies tysh's proxy,
		// strictSSL, and certificate handling — the path the rest of tysh uses.
