...
export class AgentHostStateManager extends Disposable {
	...
	/** Whether the session is owned by a throwaway tysh chat surface. */
	isEphemeralSession(session: string): boolean {
		const entry = this._sessionStates.get(session);
		return entry ? readEphemeralSessionMeta(entry.state).isEphemeral === true : false;
	}

	/** Returns the typed tysh surface metadata for a tracked session, when present. */
	 ...
	 * the session's single chat. tysh models every session as having
	 ...
	 * dispatching this action; for tysh-internal clients that happens
	 ...
	 * activity status) onto the chat channel. To preserve tysh's
