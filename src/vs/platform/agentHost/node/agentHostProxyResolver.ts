 ...
 * CAPI proxy through tysh's Electron session before spawning the Copilot SDK.
 *
...
export interface IAgentHostProxyResolver {
	 ...
	 * so the same precedence as the rest of tysh applies: `http.noProxy` →
	 * `http.proxy` setting → `HTTP(S)_PROXY` env vars → the host proxy resolution
	 * that runs in tysh (Electron session) via the reverse channel.
	 */
	...
	private _getProxyResolver(): ReturnType<typeof createProxyResolver> {
		if (!this._proxyResolver) {
			...
			const params: ProxyAgentParams = {
				// The host proxy resolution runs in tysh: reverse-call a connected
