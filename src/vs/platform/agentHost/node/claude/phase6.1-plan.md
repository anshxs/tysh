...
- **D4 sidecar-gated vs SDK-gated existence check.** CopilotAgent's `getSessionMetadata` (`copilotAgent.ts:560-590`) returns `undefined` when there's no stored sidecar — every Copilot session is born inside tysh, so the sidecar is a sound presence test. Claude cannot inherit that pattern: the same external-CLI carve-out that drives the listSessions D5 reversal also applies here. Resolution: use `_sdkService.getSessionInfo(sessionId)` as the existence test (SDK miss ⇒ undefined) and treat the overlay as decoration only.
