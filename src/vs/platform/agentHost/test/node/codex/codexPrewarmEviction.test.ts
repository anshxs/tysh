		...
		peer.push({
			...
			result: {
				data: [
					...
					{ id: 'vscode-generated', cwd: generatedWorkspace.fsPath, path: vscodeGeneratedRollout.fsPath, source: 'vscode', modelProvider: 'openai', createdAt: 5, updatedAt: 6, name: 'tysh generated' },
				],
				nextCursor: null,
			}
		});
	...
	test('client skills stay session-scoped across bundle revisions and workspaces', async () => {
		...
		const sessions = await Promise.all(scopes.map(async scope => {
			...
			entry.clientCustomizations.setClient('client', [{
				synced: { customization: { type: CustomizationType.Plugin, id: bundleUri, uri: bundleUri, name: 'tysh Synced Data' }, pluginDir },
