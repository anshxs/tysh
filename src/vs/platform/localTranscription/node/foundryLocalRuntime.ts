 ...
 * GenAI). The addon requires a newer glibc than tysh's minimum supported
 ...
 * per-target tarball of the addon + core libraries to tysh's CDN at build
 ...
 * variables (`HTTPS_PROXY`/`HTTP_PROXY`/`ALL_PROXY`, with `NO_PROXY`). tysh's
 ...
 * proxy configured only in tysh is honored here and by the native model
...
async function doEnsure(overrideDir: string, platformKey: string, download: IFoundryLocalRuntimeDownload, token: CancellationToken, onProgress?: FoundryLocalRuntimeProgress): Promise<string> {
	...
	// machine running x64 tysh under emulation, then arm64 tysh). Verify
...
export async function provisionRuntime(overrideDir: string, platformKey: string, urlTemplate: string, version: string, token: CancellationToken): Promise<void> {
	...
	// The cache is shared by the utility processes of every open tysh window,
 ...
 * mirrors the cache layout). The tarball is published to tysh's CDN by
