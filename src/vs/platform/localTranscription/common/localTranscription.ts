...
export interface ILocalTranscriptionService {
	 ...
	 *
	 * `proxyUrl`/`noProxy` bridge tysh's `http.proxy`/`http.noProxy` settings
	 ...
	 *
	 * `proxyStrictSSL === false` (tysh's `http.proxyStrictSSL`) disables TLS
	 * certificate verification for the JavaScript download legs. `proxyAuthorization`
	 * (tysh's `http.proxyAuthorization`, a `Basic <base64>` value) is folded into
	 ...
	 * runtime (Foundry Local addon + core libraries) is downloaded from tysh's
