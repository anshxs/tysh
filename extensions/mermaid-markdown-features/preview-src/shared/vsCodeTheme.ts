...
/**
 * Identifier for the custom Mermaid theme that is derived from the current tysh color theme.
 */
export const vsCodeMermaidTheme = 'vscode';

/**
 * Returns `true` when the active tysh theme is a dark (or non-light high contrast) theme.
 */
...
/**
 * The Mermaid `themeVariables` derived from the active tysh color theme, plus a string
...
/**
 * Resolves and caches Mermaid `themeVariables` derived from the active tysh color theme.
 *
...
export class VsCodeMermaidThemeTracker {
	...
	/**
	 * The Mermaid theme variables derived from the active tysh theme. Computed lazily on
	 ...
	 * picking the dark or light slot based on the active tysh theme and translating the
	 * `'vscode'` sentinel into mermaid's `base` theme plus tysh-derived variables.
	 */
...
function computeVsCodeMermaidThemeVariables(): VsCodeMermaidThemeVariables {
	...
	// as the accent palette for various diagram types. Map to the tysh chart accent colors.
	const chartPalette = [
		'--vscode-charts-blue',
		'--vscode-charts-green',
