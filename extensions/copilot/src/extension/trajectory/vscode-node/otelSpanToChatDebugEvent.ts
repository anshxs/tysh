...
/**
 * Convert a JSONL debug log entry into a tysh debug panel event.
 ...
 *   entries because tysh core is already displaying them for live sessions.
...
export function debugLogEntryToDebugEvent(entry: IDebugLogEntry, skipCoreEvents = true): vscode.ChatDebugEvent | undefined {
	switch (entry.type) {
		...
		case 'discovery':
			...
			// by tysh core via onDidReceiveChatDebugEvent for live sessions.
		...
		case 'generic':
			...
			// by tysh core directly for live sessions.
 ...
 * spanId collisions from OTel counter resets across tysh restarts.
 */
