...
/**
 *
 * We keep parsing in two phases to mirror tysh's shape:
 ...
 * workspace root. tysh's built-in detector cannot resolve that context.
 */
export class CopilotCLITerminalLinkProvider implements TerminalLinkProvider<CopilotCLITerminalLink> {
	...
	async provideTerminalLinks(context: TerminalLinkContext, token: CancellationToken): Promise<CopilotCLITerminalLink[]> {
		...
		// Match tysh's built-in MaxLineLength limit (terminalLocalLinkDetector.ts).
		...
		for (const candidate of this._detectLinkCandidates(line)) {
			// Match tysh's built-in MaxResolvedLinksInLine (terminalLocalLinkDetector.ts).
			...
			// Mirrors tysh's specialEndCharRegex (terminalLocalLinkDetector.ts).
			let trimmed = 0;
