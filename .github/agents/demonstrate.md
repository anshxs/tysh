---
name: Demonstrate
description: Agent for demonstrating tysh features
...
5. Start tysh automation using `vscode_automation_start`
...
## Pointers for Controlling tysh

- **Prefer `vscode_automation_*` tools over `browser_*` tools** when available - these are designed specifically for tysh interactions and provide more reliable control. For example:
...
**Monaco editors (used throughout tysh) DO NOT work with standard Playwright methods like `.click()` on textareas or `.fill()` / `.type()`**
