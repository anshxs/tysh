...
This is a temporary compatibility path for existing permission policies introduced before tysh 1.133.0. The migration remains in progress.
...
- **Legacy tysh setting translation** reads explicitly configured global values (policy, user, then application) from an existing setting and contributes equivalent restrictions through the declarative Agent Host bridge.
